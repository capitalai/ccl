...
static bool _key_item_init(seq* x, cap* d) {
    key* o = pobj_hold(x);
    return ARG.f_init ? ARG.f_init(o, d) : true;
}
static void _key_item_fini(seq* x, cap* d) {
    key* o = pobj_hold(x);
    if(ARG.f_fini) ARG.f_fini(o, d);
}
...
pobj_t key_init(pbuf_t b, parg_t a, pobj_t h) {
    ...
    key* o = (key*)p_buf;
    p_buf += t_size(key);  // use p_buf for seq object data
    ...
    COR.data = seq_init(p_buf, &seq_x, o);
    p_buf += seq_pass(&seq_x);  // use p_buf for seq object data
...
size_t key_loop(key* o, key_loop_f f, text_t start, text_t stop, parg_t extra) {
    cap* a   = start ? key_get(o, start) : NULL;
    ifx(a) a = key_head(o);
    cap* b   = stop ? key_get(o, stop) : NULL;
    ifx(a) b = key_tail(o);
