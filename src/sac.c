...
static void _sac_del_slot(sac* o, _sac_node* p) {  // remove pack in free list and slot
    ...
    if(p->free_p) {
        ...
        if(p->free_n) p->free_n->free_p = NULL;
        else o->map &= ~(1 << c);
    }
...
static bool sac_drop(bag* b, cap* p) {
    ...
    _sac_node* t = (_sac_node*)p;
    ccf(t->free_id);  // id != 0 -> free
...
static void sac_fini(bag* b) { bag_fini(psac(b)->pick); }
