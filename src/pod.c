...
pobj_t pod_init(pbuf_t b, parg_t a, pobj_t h) {
    ...
    pod* o = (pod*)p_buf;
    p_buf += t_size(pod);  // use p_buf for pod object data
...
static inline cap* _pod_new_item(pod* o) {
    cap* x = ARG.f_push ? ARG.f_push(o, NULL, o->d_work.d) : o->d_work.d;
    cxx(x);
...
static inline bool _pod_set_item(pod* o, cap* t) {
    ...
    cap* x = ARG.f_push(o, *p, o->d_work.d);
    cxf(x);
...
static inline bool _pod_add_ba(pod* o, _pod_node* n) {
    bool t = _pod_add(o, n->ba);
    ifx(o->d_work.m) return t;  // add to bb
...
bool pod_add(pod* o, cap* d, pod_p* p) {
    ...
    bool t = _pod_add(o, COR.root);
    ifx(o->d_work.m) return t;
...
static bool _pod_del(pod* o, _pod_node* n) {
    ...
    if(n->b) {
        ...
        ifz(c) {
            ...
            if(n->bc) {
                n->b = _pod_move_min(o, n->bc);
                POD_DROP(n->bc);
            } else if(n->bb) {
                n->b = _pod_move_max(o, n->bb);
                POD_DROP(n->bb);
            } else {
                n->b  = n->c;
                n->bc = n->ac;
        ...
        if(c > 0) {
            if(n->c) {
                ...
                ifz(c) {
                    ...
                    if(n->ac) {
                        n->c = _pod_move_min(o, n->ac);
                        POD_DROP(n->ac);
                    } else if(n->bc) {
                        n->c = _pod_move_max(o, n->bc);
                        POD_DROP(n->bc);
                    } else n->c = NULL;
...
size_t pod_loop(pod* o, pod_loop_f f, data_t start, data_t stop, parg_t extra) {
    ...
    if(start) pod_find_next(o, &p1, start);
    else pod_head(o, &p1);
    if(stop) pod_find_prev(o, &p2, stop);
    else pod_tail(o, &p2);
...
size_t pod_hoop(pod* o, pod_loop_f f, data_t start, data_t stop, parg_t extra) {
    ...
    if(start) pod_find_prev(o, &p1, start);
    else pod_tail(o, &p1);
    if(stop) pod_find_next(o, &p2, stop);
    else pod_head(o, &p2);
...
void pod_dump(pod* o, pod_dump_f f) { _pod_dump(o, f, COR.root, 0); }
