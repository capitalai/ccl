...
bool new_take(take_f f, void* e) {
    ccf(fp_take);
    fp_take = f;
    dp_take = e;
    return true;
}
bool new_drop(drop_f f, void* e) {
    ccf(fp_drop);
    fp_drop = f;
    dp_drop = e;
    return true;
}
bool new_note(note_f f, void* e) {
    ccf(fp_note);
    fp_note = f;
    dp_note = e;
    return true;
}
bool new_term(term_f f, void* e) {
    ccf(fp_term);
    fp_term = f;
    dp_term = e;
    return true;
}
bool new_fail(fail_f f, void* e) {
    ccf(fp_fail);
    fp_fail = f;
    dp_fail = e;
    return true;
...
int fail(text_t c, text_t v, text_t f, text_t m, int l) {
    return fp_fail ? fp_fail(c, v, f, m, l, dp_fail) : _fail(c, v, f, m, l);
}
...
pbuf_t _take(size_t s) {
    ...
    pbuf_t r = malloc(s);
    cxx(r);  // malloc() will not clear memory content
...
uint_t data_hash(data_t a, size_t s) {
    ...
    if(s > 8) {
        ...
        ulli_t t = 0;
        data_copy(&t, p, s);  // use data_copy() for safety
