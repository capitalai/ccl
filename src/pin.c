...
pobj_t pin_init(pbuf_t b, parg_t a, pobj_t h) {
    ...
    pin* o = (pin*)p_buf;
    p_buf += t_size(pin);  // object data block
...
cap* pin_ask(pin* o, size_t* n, size_t s, size_t e) {
    ...
    ifx(COR.root) {
        ...
        if(COR.c_item == COR.max_count) {
            ...
            COR.root     = cap_data(PIN_TAKE(_pin_node));
            DATA_WIPE(COR.root, _pin_node);
    ...
    size_t s1 = MAX(ARG.s_item, s);
    if(s1 > ARG.s_item) COR.can_swap = false;
...
static bool _pin_swap(pin* o, size_t a, size_t b) {
    ...
    void* pc = COR.p_tmp;
    void* pa = cap_data(pin_get(o, a));
    cxf(pa);
    void* pb = cap_data(pin_get(o, b));
    cxf(pb);
...
static void _pin_qsort(pin* o, pin_comp_f f, size_t a, size_t b, parg_t e) {
    if(a < b) {
        ...
        for(size_t i = a; i < b; i++)
            if(f(o, cap_data(pin_get(o, i)), pb, e) <= 0) _pin_swap(o, ++c, i);
