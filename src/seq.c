...
pobj_t seq_init(pbuf_t b, parg_t a, pobj_t h) {
    ...
    seq* o = (seq*)p_buf;
    p_buf += t_size(seq);  // use p_buf for seq object data
...
static size_t _seq_loop_fini(pin* x, pin_l* p) {
    _seq_node* t = pin_head_data(x, p->d);
    cnz(t->stat == SEQ_STAT_USED);
...
static bool _seq_remove(seq* o, cap* d) {
    ...
    if(p->prev) SEQ_NODE(p->prev)->next = p->next;
    else COR.head = p->next;
    if(p->next) SEQ_NODE(p->next)->prev = p->prev;
    else COR.tail = p->prev;
...
bool seq_put(seq* o, cap* a, cap* d) {
    ...
    _seq_node *p = SEQ_NODE(a), *q = SEQ_NODE(d);
...
bool seq_ins(seq* o, cap* b, cap* d) {
    ...
    _seq_node *p = SEQ_NODE(b), *q = SEQ_NODE(d);
