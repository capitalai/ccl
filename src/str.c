...
str* str_init(str_pool* p, text_t source) {
    ...
    size_t n = source ? text_size(source, SIZE_MAX) : 0;
    if(n < p->s) n = p->s;
...
str* str_add(str* s, text_t d, size_t n) {
    ...
    if(ss > bs) {
        size_t n = CEIL(ss, def_str_size);
        n += n / def_str_add_rate;
...
str* str_set(str* s, text_t d, size_t n) {
    ...
    if(ts > bs) {
        ...
        size_t c = CEIL(ts, def_str_size);
        c += c / def_str_add_rate;
