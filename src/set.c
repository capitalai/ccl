...
size_t set_pass(parg_t a) {
    ...
    size_t pin_s = pin_pass(&pin_x);
    czz(pin_s);
    size_t pod_s = pod_pass(&pod_x);
    czz(pod_s);
...
pobj_t set_init(pbuf_t b, parg_t a, pobj_t h) {
    ...
    set* o = (set*)p_buf;
    p_buf += t_size(set);  // use p_buf for set object data
    ...
    COR.data = pin_init(p_buf, &pin_x, o);
    p_buf += pin_pass(&pin_x);  // use p_buf for pin object data
    COR.count  = 0;
    COR.p_free = NULL;
    ...
    switch(ARG.s_index) {
    case SET_INDEX_TEXT:
        f_hash     = _set_comp_hash;
        f_find     = _set_find_hash;
        COR.f_comp = _set_comp_text;
        break;
    case SET_INDEX_CHAR:
        f_hash     = _set_comp_char;
        f_find     = _set_find_char;
        COR.f_comp = NULL;
        break;
    case SET_INDEX_UINT:
        f_hash     = _set_comp_uint;
        f_find     = _set_find_uint;
        COR.f_comp = NULL;
        break;
    case SET_INDEX_LONG:
        f_hash     = _set_comp_long;
        f_find     = _set_find_long;
        COR.f_comp = NULL;
        break;
    case SET_INDEX_FLOAT:
        f_hash     = _set_comp_float;
        f_find     = _set_find_float;
        COR.f_comp = NULL;
        break;
    default:
        f_hash     = _set_comp_hash;
        f_find     = _set_find_hash;
        COR.f_comp = _set_comp_data;
        break;
    }
...
static cap* _set_push(pod* x, cap* a, cap* b) {
    ...
    if(COR.f_comp)
        while(COR.f_comp(na->index, nb->index, s)) {  // not same index
...
static cap* _set_pull(pod* x, data_t a, cap* b) {
    ...
    if(COR.f_comp)
        while(COR.f_comp(i, nb->index, sb)) {
            t = b;
            b = nb->hash_next;
            ifx(b) return r;
...
bool set_add(set* o, parg_t i, parg_t d) {
    ...
    switch(ARG.s_index) {
    case SET_INDEX_TEXT:
        n->hash.hash = text_hash(i);
        text_copy(n->index, i, s_index);
        break;
    ...
    default:
        n->hash.hash = data_hash(i, s_index);
        data_copy(n->index, i, s_index);
        break;
    }
...
cap* set_get(set* o, parg_t i) {
    ...
    if(COR.f_comp)
        while(COR.f_comp(an->index, i, as)) {
            a = an->hash_next;
            cxx(a);
...
static size_t _set_loop_index(set* o, cap* a) {
    ...
    if(COR.f_comp)
        while(COR.f_comp(na->index, o->d_work.d_loop->i, sa)) {
            cxz(na->hash_next);
